...
Button.prototype['toggle'] = function () {
  ...
  if (rootElement) {
    var input = this._element.querySelector(Button._Selector.INPUT)
    if (input) {
      if (input.type == 'radio') {
        if (input.checked && this._element.classList.contains(Button._ClassName.ACTIVE)) {
          triggerChangeEvent = false
        } else {
          var activeElement = rootElement.querySelector(Button._Selector.ACTIVE)
          if (activeElement) activeElement.classList.remove(Button._ClassName.ACTIVE)
        }
      }

      if (triggerChangeEvent) {
        input.checked = !this._element.classList.contains(Button._ClassName.ACTIVE)
        $(this._element).trigger('change')
      }
    }
  } else {
    this._element.setAttribute('aria-pressed', !this._element.classList.contains(Button._ClassName.ACTIVE))
  }

  if (triggerChangeEvent) {
    this._element.classList.toggle(Button._ClassName.ACTIVE)
  }
}
...
$(document)
  .on('click.bs.button.data-api', Button._Selector.DATA_TOGGLE_CARROT, function (event) {
    ...
    if (!button.classList.contains(Button._ClassName.BUTTON)) {
  ...
  .on('focus.bs.button.data-api blur.bs.button.data-api', Button._Selector.DATA_TOGGLE_CARROT, function (event) {
    ...
    button.classList.toggle(Button._ClassName.FOCUS, /^focus(in)?$/.test(event.type))
  })
