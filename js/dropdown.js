...
Dropdown._clearMenus = function (opt_event) {
  ...
  var backdrop = document.querySelector(Dropdown._Selector.BACKDROP)
  if (backdrop) {
    backdrop.parentNode.removeChild(backdrop)
  }

  var toggles = document.querySelectorAll(Dropdown._Selector.DATA_TOGGLE)

  for (var i = 0; i < toggles.length; i++) {
    var parent = Dropdown._getParentFromElement(toggles[i])
    var relatedTarget = { 'relatedTarget': toggles[i] }

    if (!parent.classList.contains(Dropdown._ClassName.OPEN)) {
      continue
    }
    ...
    parent.classList.remove(Dropdown._ClassName.OPEN)
    $(parent).trigger(Dropdown._Event.HIDDEN, relatedTarget)
  }
}
...
Dropdown._dataApiKeydownHandler = function (event) {
  ...
  if (this.disabled || this.classList.contains(Dropdown._ClassName.DISABLED)) {
    return
  }

  var parent  = Dropdown._getParentFromElement(this)
  var isActive = parent.classList.contains(Dropdown._ClassName.OPEN)

  if ((!isActive && event.which != 27) || (isActive && event.which == 27)) {
    if (event.which == 27) {
      var toggle = parent.querySelector(Dropdown._Selector.DATA_TOGGLE)
  ...
  var items = document.querySelectorAll(Dropdown._Selector.VISIBLE_ITEMS)
  items = Array.prototype.slice.apply(items).filter(function (item) {
...
Dropdown.prototype['toggle'] = function () {
  ...
  if ('ontouchstart' in document.documentElement && !$(parent).closest(Dropdown._Selector.NAVBAR_NAV).length) {
    ...
    dropdown.classList.add(Dropdown._ClassName.BACKDROP)
  ...
  parent.classList.toggle(Dropdown._ClassName.OPEN)
